...
/**
 * Decimal number toString comparison: all Java idiomatic ways of writing
 ...
 * - ClassicFloatWriter (digit-by-digit, scratch buffer)
 * - String.format variants
 *
 * <p>Run:
 * {@code java -cp <classpath> org.openjdk.jmh.Main
 * hr.hrg.dialog.core.DecimalNumberToStringBenchmark -wi 3 -i 5 -f 1}
 */
...
@Fork(1)
@State(Scope.Thread)
public class DecimalNumberToStringBenchmark {
    ...
    @Benchmark
    public int floatToStringWithFormat(Blackhole bh) {
        float value = floatValues[indexFloat++ & (N - 1)];
    ...
    @Benchmark
    public int doubleToStringWithFormat(Blackhole bh) {
        double value = doubleValues[indexDouble++ & (N - 1)];
        byte[] bytes = String.format("%f", value).getBytes(StandardCharsets.UTF_8);
        bh.consume(bytes[bytes.length - 1]);
        return bytes.length;
    ...
    @Benchmark
    public int floatToStringCompact(Blackhole bh) {
        float value = floatValues[indexFloat++ & (N - 1)];
    ...
    @Benchmark
    public int doubleToStringCompact(Blackhole bh) {
        double value = doubleValues[indexDouble++ & (N - 1)];
        byte[] bytes = String.valueOf(value).getBytes(StandardCharsets.UTF_8);
        bh.consume(bytes[bytes.length - 1]);
        return bytes.length;
    ...
    public int longToStringWithFormat(Blackhole bh) {
        long value = indexDouble++ & (N - 1); // reuse doubleValues for long format test
        byte[] bytes = String.format("%d", value).getBytes(StandardCharsets.UTF_8);
        bh.consume(bytes[bytes.length - 1]);
        return bytes.length;
    }

    // Note: classicFloatWriter benchmarks removed - use Integer/Float.toString() as baseline

    @Override
    public String toString() {
        return "DecimalNumberToStringBenchmark";
    }
