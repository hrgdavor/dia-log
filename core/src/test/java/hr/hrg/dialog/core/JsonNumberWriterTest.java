...
class JsonNumberWriterTest {
    ...
    class FloatTests {
        ...
        void testZeroes() throws IOException {
            JsonNumberWriter.writeFloat(out, 0.0f);
    ...
    class DoubleTests {
        ...
        @Test
        void testZeroes() throws IOException {
            JsonNumberWriter.writeDouble(out, 0.0);
            assertEquals("0.0", getWrittenString());
        ...
        void testNonFiniteDoublesProduceJsonNull() throws IOException {
            JsonNumberWriter.writeDouble(out, Double.NaN);
            assertEquals("null", getWrittenString());

            out.reset();
            JsonNumberWriter.writeDouble(out, Double.POSITIVE_INFINITY);
            assertEquals("null", getWrittenString());

            out.reset();
            JsonNumberWriter.writeDouble(out, Double.NEGATIVE_INFINITY);
            assertEquals("null", getWrittenString());
        }
    }
    ...
    class IntAndLongTests {
        ...
        void testInts(int val) throws IOException {
            JsonNumberWriter.writeInt(out, val);
        ...
        void testLongs(long val) throws IOException {
            JsonNumberWriter.writeLong(out, val);
            assertEquals(String.valueOf(val), getWrittenString());
        }
    }
    ...
    class WriteNumberRouterTests {
        ...
        void testNullNumber() throws IOException {
            JsonNumberWriter.writeNumber(out, null);
            assertEquals("null", getWrittenString());
        }

        @Test
        void testBoxedTypes() throws IOException {
            JsonNumberWriter.writeNumber(out, Integer.valueOf(42));
            assertEquals("42", getWrittenString());

            out.reset();
            JsonNumberWriter.writeNumber(out, Float.valueOf(3.14f));
            assertEquals(3.14f, Float.parseFloat(getWrittenString()), 1e-6f);

            out.reset();
            JsonNumberWriter.writeNumber(out, Double.valueOf(2.718281828));
