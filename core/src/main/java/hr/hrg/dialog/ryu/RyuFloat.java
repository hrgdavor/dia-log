...
package hr.hrg.dialog.ryu;

import java.math.BigInteger;

/**
 * An implementation of Ryu for {@code float}, ported from the Apache-2.0
 * licensed reference implementation by Ulf Adams (ryu-java,
 * <a href="https://github.com/ulfjack/ryu">https://github.com/ulfjack/ryu</a>).
 * <p>
 * Ryu produces the shortest decimal representation that round-trips to the
 * original {@code float}, using the {@code Float.toString} formatting rules
 * (decimal notation for values in {@code [1E-3, 1E7)}, scientific notation
 * otherwise, at least two significant digits, round-to-even tie breaking).
 * <p>
 * Adapted to write the ASCII digits directly into a caller-provided
 * {@code byte[]} so no {@code String} (or any other) object is allocated on
 * the formatting path. The power-of-5 lookup tables are generated once at
 * class initialization.
 * <p>
 * {@code result} must have room for at most 15 bytes at {@code index} (e.g.
 * {@code "-1.17549435E-38"}).
 */
public final class RyuFloat {
    ...
    static {
        ...
        for (int i = 0; i < Math.max(POW5_SPLIT.length, POW5_INV_SPLIT.length); i++) {
    ...
    /**
     * Writes the shortest round-trip decimal representation of {@code value}
     * into {@code result} starting at {@code index}, following
     * {@link Float#toString(float)} semantics. Returns the number of bytes
     * written.
     */
    public static int writeFloat(float value, byte[] result, int index) {
        // Step 1: Decode the floating point number, and unify normalized and subnormal cases.
        // First, handle all the trivial cases.
        if (Float.isNaN(value)) {
            return writeAscii(result, index, "NaN");
        }
        if (value == Float.POSITIVE_INFINITY) {
            return writeAscii(result, index, "Infinity");
        }
        ...
        int mv = 4 * m2;
        int mp = 4 * m2 + 2;
        int mm = 4 * m2 - ((m2 != (1 << FLOAT_MANTISSA_BITS)) || (ieeeExponent <= 1) ? 2 : 1);
        e2 -= 2;
    ...
    private static int writeAscii(byte[] result, int index, String s) {
        for (int i = 0; i < s.length(); i++) {
            result[index + i] = (byte) s.charAt(i);
        }
        return s.length();
    }
