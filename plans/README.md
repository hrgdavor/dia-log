...
The `errHash` field is emitted as **`"errHash"`** — a single top-level JSON key (not nested under an `"err"` object), with its value being the Wyhash64 fingerprint of the sanitized stack trace. It is not named `"hash"`, `"error hash"`, or placed under an `"errors"`/`"exception"` container; it is written directly as `KEY_ERR_HASH` alongside `errClass`, `errMessage`, and `stack`. This differs from the original roadmap schema that documented `"hash": <value>` at top level — the current code emits a flat field named `"errHash"`.
